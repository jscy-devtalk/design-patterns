...
## 프로토타입 패턴 적용 하기 
우선 프로토타입 패턴의 구성요소대로 인터페이스를 만든다. 왜 인터페이스가 필요한지에 대해선 코드를 통해 알아 보자.
```java
...
public class Cat implements Cloneable {
  ...
  @Override
  public Animal clone() {
    return new Cat(this);   // 팩토리 메소드로 깊은복사 한다.
  }
}
```
...
public class Dog implements Cloneable {
  ...
  @Override
  public Animal clone() {
    return new Dog(this);   // 팩토리 메소드로 깊은복사 한다.
  }
}
```
...
리플랙션 관련해서는 다음 글에서 정리했다.
https://zhtmr.github.io/java/proxy-pattern/
...
## 정리
프로토타입 패턴은 다른 패턴에 비해 간단하다. 패턴의 자체가 객체 복제라는 명확한 목표가 있기 때문이다. 
